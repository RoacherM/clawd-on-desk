...
> Supports Windows 11, macOS, and Ubuntu/Linux. Requires Node.js. Works with **Claude Code**, **Codex CLI**, **Copilot CLI**, **Gemini CLI**, **Cursor Agent**, **CodeBuddy**, **Kiro CLI**, **Kimi Code CLI (Kimi-CLI)**, **Hermes**, and **opencode**.
...
- **Hermes** — shell hooks via `~/.hermes/config.yaml` (`hooks:` entries) (registered automatically when Clawd starts, or run `npm run install:hermes-hooks`)
