...
| **Hermes: no permission bubble** | Hermes shell hooks currently report state only. Tool approvals remain in Hermes' own terminal/TUI. |
| **Hermes: `hooks:` entries referencing `hermes-hook.js` are owned by Clawd** | Clawd auto-syncs Hermes hooks on every startup (and from `npm run install:hermes-hooks`). Any hook list item whose `command` references `hermes-hook.js` is treated as Clawd-owned and rewritten; other user hook entries and non-hook config are preserved. |
