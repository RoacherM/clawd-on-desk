...
| **Hermes：无权限气泡** | Hermes shell hooks 目前只上报状态；工具审批仍在 Hermes 自己的终端 / TUI 中处理。 |
| **Hermes：引用 `hermes-hook.js` 的 `hooks:` 条目由 Clawd 接管** | Clawd 每次启动（以及执行 `npm run install:hermes-hooks`）都会自动同步 Hermes hooks。凡是 `command` 里引用 `hermes-hook.js` 的 hook 列表项，都会被视为 Clawd-owned 并重写；其他用户 hook 条目和非 hook 配置会保留。 |
