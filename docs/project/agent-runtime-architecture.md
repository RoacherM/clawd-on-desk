...
Hermes 状态同步（hook-only，config.yaml）：
  Hermes 触发 shell hook 事件
    → hooks/hermes-hook.js（hook_event_name → agents/hermes.js 映射 → HTTP POST）
    → 同上状态机（agent_id: hermes）
  Hook 注册到 ~/.hermes/config.yaml 的 hooks: 条目；Clawd 启动时会自动同步这些条目。
...
- `server.js` 启动后异步同步 Claude / Gemini / Cursor / CodeBuddy / Kiro / Kimi / Hermes hooks 和 opencode plugin
